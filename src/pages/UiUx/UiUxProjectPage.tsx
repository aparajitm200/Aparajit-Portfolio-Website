...
import { CinematicOverview, InsightQuote, BentoStats, FlowSteps, DesignSystemBlock } from '../../components/ui/CaseStudyBlocks'
import type { CategorySlug } from '../../types/portfolio'

const ALL_UIUX_IMAGES = Object.keys(
  import.meta.glob('/public/images/uiux/**/*.{png,jpg,jpeg,webp,gif}', { eager: true })
)

export function UiUxProjectPage() {
  const { projectSlug } = useParams<{ projectSlug: string }>()
  const project = getProjectBySlug('ui-ux' as CategorySlug, projectSlug ?? '')
  ...
  if (project) {
    const prefix = `/public/images/uiux/${project.slug}/`
    ALL_UIUX_IMAGES.forEach((path) => {
  ...
  return (
    <PageTransition>
      <article style={{ paddingTop: 'calc(var(--space-20) + 60px)' }}>
        ...
        {(() => {
          const standaloneSections = project?.sections?.filter(
            s => !dynamicSections.some(ds => ds.title.toLowerCase() === s.title.toLowerCase())
          ) || []
          
          if (standaloneSections.length === 0) return null
          
          return (
            <div className="container" style={{ marginBottom: 'var(--space-24)' }}>
              <div style={{
                display: 'grid',
                gridTemplateColumns: '1fr',
                gap: 'var(--space-12)',
                ...
                {standaloneSections.map((section, idx) => {
                  switch (section.layout) {
                    case 'cinematic-overview':
                      return <CinematicOverview key={idx} title={section.title} description={section.description} />
                    case 'insight-quote':
                      return <InsightQuote key={idx} quote={section.description!} author={section.contentData?.author} />
                    case 'bento-stats':
                      return <BentoStats key={idx} title={section.title} items={section.contentData?.items || []} />
                    case 'flow-steps':
                      return <FlowSteps key={idx} title={section.title} steps={section.contentData?.steps || []} />
                    case 'design-system':
                      return <DesignSystemBlock key={idx} title={section.title} data={section.contentData} />
                    default:
                      // Default text block
                      return (
                        <section key={`text-${idx}`} style={{ marginBottom: 'var(--space-12)' }}>
                          <h2 className="t-heading-lg" style={{ marginBottom: 'var(--space-4)' }}>
                            {section.title}
                          </h2>
                          <p className="t-body-md" style={{ color: 'var(--color-text-muted)', lineHeight: 'var(--leading-relaxed)' }}>
                            {section.description}
                          </p>
                        </section>
                      )
